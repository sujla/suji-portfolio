import{p as _o,a as eo}from"./projects-KnV3ap8A.js";const Uo=[{id:"store-finder",type:"e-commerce",href:"./projects/store-finder-renewal/",title:"Store Finder Renewal for Business Impact",companyLabel:"CJ Olive Young",year:2024,deviceType:"mobile",media:"store-guide",projectMedia:"./assets/common/store-guide-main.avif",cta:{label:"View Case Study",href:"./projects/store-finder-renewal/",newTab:!1}},{id:"article-studio",type:"enterprise",href:"./projects/modular-editorial-system/",title:"Editorial Workflow: From Weeks to Minutes",companyLabel:"CJ Olive Young",year:2024,deviceType:"web",projectMedia:"./assets/common/article-studio-main.avif",cta:{label:"View Case Study",href:"https://sujikweon.framer.website/article-studio",newTab:!0}},{id:"public-transport",type:"mobility",types:["ai","mobility"],title:"Public Transport App in SG",companyLabel:"Taurus Labs",year:2026,deviceType:"mobile",modalDescription:"Finding the right MRT exit and exploring transit routes on a full-screen map still felt harder than it should be. I built BusMrtSG to solve those transit-specific problems using AI coding tools and a serverless architecture."},{id:"cta-enhancement",type:"e-commerce",href:"./projects/cta-enhancement/",title:"Pickup Order UX Enhancement",companyLabel:"CJ Olive Young",year:2025,deviceType:"mobile",projectMedia:"./assets/common/cta-enh-main.avif",cta:{label:"View Case Study",href:"https://sujikweon.framer.website/cta-enhancement",newTab:!0}},{id:"perp-dex",type:"fintech",title:"Clearer Control for Perp Trading",companyLabel:"Taurus Labs",year:2025,deviceType:"mobile",modalDescription:"I redesigned the trading and wallet experience to streamline key workflows, helping users act with greater speed and confidence while improving portfolio visibility and transaction clarity."}],Jo=(o,a,u)=>{if(!o)return;const h=760,S="portfolioPfModalProject",O=Object.entries({AI:"ai","Enterprise/SaaS":"enterprise",Fintech:"fintech","E-commerce":"e-commerce",Mobility:"mobility"}).map(([e,t])=>({label:e,value:t}));let $=null;const de=e=>{const t=[e.companyLabel,e.year].filter(Boolean);return t.length?`<span>${t.join(" · ")}</span>`:""},J=e=>{window.gtag?.("event","pf_work_card_click",{project_id:e.id,project_title:e.title})},C=()=>window.history.state?.[S]||"",lo=e=>{const t=window.history.state&&typeof window.history.state=="object"?window.history.state:{};window.history.pushState({...t,[S]:e},"")},rt=e=>e.deviceType==="web"?`
      ...
      `:'<div class="pf-work-empty" aria-hidden="true"></div>',co=[{source:"./assets/public-transport/bus-route.mp4",poster:"./assets/public-transport/bus-route-poster.jpg",label:"Bus timings & route"},{source:"./assets/public-transport/mrt-route.mp4",poster:"./assets/public-transport/mrt-route-poster.jpg",label:"Simple & shortest MRT route options"},{source:"./assets/public-transport/google-map-share.mp4",poster:"./assets/public-transport/google-map-share-poster.jpg",label:"Locations shared via Google Maps"}],st=e=>{const t=co[e-1];return`
    ...
    `},po=e=>e.projectMedia?`<img class="pf-modal-project-media" src="${e.projectMedia}" alt="" />`:e.id==="public-transport"?st(1):rt(e),uo=[{start:0,end:5},{start:18,end:26}],fo=(e,t)=>{const s=e.id==="public-transport"?st(t+2):"",n=e.id==="perp-dex"?[`
    ...
    `;return t?`<div class="pf-work pf-work--${e.id} pf-work--clone" aria-hidden="true">${l}</div>`:`<a class="pf-work pf-work--${e.id}" href="${e.href}" data-project-types="${s.join(" ")}" draggable="false" aria-label="${u(e.title)} project detail">${l}</a>`},it=e=>e.button===0&&!e.defaultPrevented&&!e.metaKey&&!e.ctrlKey&&!e.shiftKey&&!e.altKey,nt=(e,t)=>it(e)&&!t.target&&!t.hasAttribute("download"),at=()=>{const e=window.innerWidth<=600?12:16;return{top:e,left:e,width:window.innerWidth-e*2,height:window.innerHeight-e*2}},lt=()=>"24px",pe=(e,t)=>{Object.assign(e.style,{top:`${t.top}px`,left:`${t.left}px`,width:`${t.width}px`,height:`${t.height}px`})},ct=(e,t)=>{const s=t.width?e.width/t.width:1,n=t.height?e.height/t.height:1,l=e.left-t.left,f=e.top-t.top;return`translate3d(${l}px, ${f}px, 0) scale(${s}, ${n})`},yo=()=>{const e=[...o.querySelectorAll(".pf-work video")],t=e.filter(s=>!s.paused&&!s.ended);return e.forEach(s=>s.pause()),t},bo=e=>{e.forEach(t=>{t.isConnected&&t.play().catch(()=>{})})},dt=(e,t,{pushHistory:s=!0}={})=>{if($)return;s&&lo(t.id);const n=yo(),l=e.querySelector(".pf-work-card")||e,f=l.getBoundingClientRect(),m=window.getComputedStyle(l).borderRadius,T=at(),L=lt(),y=document.createElement("div"),b=document.createElement("div"),r=document.createElement("div"),k=`pf-modal-${t.id}-title`,i=window.matchMedia("(prefers-reduced-motion: reduce)").matches,g=i?1:h,w=t.cta?`
      ...
      `:"",A=t.modalDescription?`<p class="pf-modal-description">${t.modalDescription}</p>`:"",E=t.id==="perp-dex",Wt=t.id==="public-transport",Ht=E?4:Wt?2:3,Ho=Array.from({length:Ht},(d,v)=>fo(t,v)),Bo=Array.from({length:Ht},(d,v)=>`
        <div class="pf-modal-bento-placeholder pf-modal-bento-placeholder--${v+1} pf-modal-bento-skeleton"></div>
      `),ie=Wt,we=!t.cta&&!E&&!ie,Ge=t.deviceType==="web"&&!ie&&!we,Bt=["cta-enhancement","store-finder"].includes(t.id),Dt=d=>{const v=Bt?d[0]:d.slice(0,2).join("");return E?`
          ...
          ${d[3]}
        `:ie||we?d.join(""):Ge?`<div class="pf-modal-bento-side">${d.slice(0,2).join("")}</div>`:`
      ...
      `,zo=`
      <section class="pf-modal-bento-section${Ge?" pf-modal-bento-section--web":""}${ie?" pf-modal-bento-section--three-up":""}${we?" pf-modal-bento-section--four-up":""}${E?" pf-modal-bento-section--perp-dex":""}" aria-hidden="true">
        ${Oo}
        ${Do}
      </section>
    `,No=`
      <section class="pf-modal-bento-section pf-modal-bento-section--skeleton${Ge?" pf-modal-bento-section--web":""}${ie?" pf-modal-bento-section--three-up":""}${we?" pf-modal-bento-section--four-up":""}${E?" pf-modal-bento-section--perp-dex":""}" aria-hidden="true">
  ...
  `,document.querySelector("[data-pf-work-cursor]")?.remove();const P=document.createElement("span");P.className="pf-work-cursor-label",P.dataset.pfWorkCursor="",P.setAttribute("aria-hidden","true"),P.innerHTML="<span>View Details</span>",document.body.append(P);const pt=[...Q?[...Q.querySelectorAll("[data-project-type-filter]")]:[],...M.querySelectorAll("[data-project-type-filter]")],ko=Q?.querySelector(".pf-type-filter"),ut=document.documentElement,j=M.querySelector(".pf-floating-filter-scroll"),Me=[...M.querySelectorAll("[data-project-type-filter]")],To=[...o.querySelectorAll(".pf-work[data-project-types]")],ft=o.querySelector(".pf-work-grid"),mt=o.querySelector(".pf-filter-empty");let z="",gt=0,ht=0,wt=0,ue=!1;const Eo=()=>{window.clearTimeout(ht),window.clearTimeout(wt)},Mo=e=>{if(e!==ue){if(ue=e,Eo(),!e){M.classList.remove("is-visible"),wt=window.setTimeout(()=>{ue||ut.classList.remove("is-footer-yielding-to-filter")},220);return}ut.classList.add("is-footer-yielding-to-filter"),ht=window.setTimeout(()=>{ue&&M.classList.add("is-visible")},220)}},fe=()=>{if(!j||!W.matches){M.classList.remove("has-scroll-left","has-scroll-right");return}const e=Math.max(0,j.scrollWidth-j.clientWidth);M.classList.toggle("has-scroll-left",j.scrollLeft>1),M.classList.toggle("has-scroll-right",j.scrollLeft<e-1)},vt=()=>{if(!j||!W.matches)return;const e=Me.find(l=>l.getAttribute("aria-pressed")==="true"),t=Me.indexOf(e);if(!e)return;const s=Math.max(0,j.scrollWidth-j.clientWidth),n=t<2?0:t>=Me.length-2?s:e.offsetLeft-(j.clientWidth-e.offsetWidth)/2;j.scrollTo({left:n,behavior:"smooth"})},$e=()=>{const e=o.getBoundingClientRect().bottom,t=window.innerHeight*Lo;Mo(window.scrollY>So&&e>t)},$o=()=>{const e=Math.max(0,o.getBoundingClientRect().top+window.scrollY-10);window.scrollTo({top:e,behavior:"smooth"})},yt=e=>{z=e;let t=0;pt.forEach(s=>{s.setAttribute("aria-pressed",String(s.dataset.projectTypeFilter===z))}),vt(),ko?.classList.toggle("is-filtering",!!z),To.forEach(s=>{const n=s.dataset.projectTypes.split(" "),l=!z||n.includes(z);s.hidden=!l,l&&(t+=1)}),ft?.classList.toggle("is-single-result",t===1),mt&&(mt.hidden=t>0)};pt.forEach(e=>{e.addEventListener("click",()=>{const t=document.documentElement,s=e.dataset.projectTypeFilter,n=!M.contains(e)&&z===s;window.clearTimeout(gt),t.style.overflowAnchor="none",yt(n?"":s),$o(),gt=window.setTimeout(()=>{t.style.removeProperty("overflow-anchor")},700)})}),window.addEventListener("scroll",$e,{passive:!0}),window.addEventListener("resize",()=>{$e(),fe()},{passive:!0}),j?.addEventListener("scroll",fe,{passive:!0}),W.addEventListener?.("change",()=>{fe(),vt()}),$e(),yt(""),fe();const c=o.querySelector("[data-pf-work-rail]"),Ce=c?[...c.querySelectorAll(".pf-work-set")]:[],bt=o.querySelector(".pf-work-set--primary .pf-work"),p=o.querySelector("[data-pf-work-rail-secondary]"),me=p?[...p.querySelectorAll(".pf-work-set")]:[],St=me[1]?.querySelector(".pf-work:nth-child(4)");o.querySelector("[data-mobile-pf-work-rail]"),o.querySelector("[data-mobile-pf-work-rail-secondary]");const Co=80,Po=-80,Pe=32,xo=12,Lt=180,kt=600;let xe,Re=0,Ae=0,x=0,qe=Pe,je=!1,Z=!1,Tt=0,Fe=0,ee=!1,ge=!1,te=!1,Ie=0,H=0,N=0,I=0,Y=0,R=0,We=0,oe=!1,Et=0,He=0,re=!1,he=!1,se=!1,Be=0;const Mt=({preservePosition:e=!1}={})=>{if(!c||!bt||Ce.length<3)return;const t=H,s=N,n=t?(c.scrollLeft-s)/t:0,l=c.getBoundingClientRect(),f=bt.getBoundingClientRect(),m=c.scrollLeft+f.left-l.left;H=Ce[2].getBoundingClientRect().left-Ce[1].getBoundingClientRect().left,N=m-(c.clientWidth-f.width)/2-(W.matches?Co:0),c.scrollLeft=e?N+n*H:N,x=c.scrollLeft},De=()=>{if(!c||!H)return 0;const e=N-H*.5,t=N+H*.5;let s=c.scrollLeft;for(;s<e;)s+=H;for(;s>=t;)s-=H;const n=s-c.scrollLeft;return Math.abs(n)>.5&&(c.scrollLeft=s),n},$t=({preservePosition:e=!1}={})=>{if(!p||!St||me.length<3||!p.clientWidth)return;const t=I,s=Y,n=t?(p.scrollLeft-s)/t:0,l=p.getBoundingClientRect(),f=St.getBoundingClientRect(),m=p.scrollLeft+f.left-l.left;I=me[2].getBoundingClientRect().left-me[1].getBoundingClientRect().left,Y=m-(p.clientWidth-f.width)/2-Po,p.scrollLeft=e?Y+n*I:Y,R=p.scrollLeft},Ve=()=>{if(!p||!I)return 0;const e=Y-I*.5,t=Y+I*.5;let s=p.scrollLeft;for(;s<e;)s+=I;for(;s>=t;)s-=I;const n=s-p.scrollLeft;return Math.abs(n)>.5&&(p.scrollLeft=s),n},Oe=()=>{x=c?.scrollLeft??x,Ae=performance.now()+200},Ct=()=>{R=p?.scrollLeft??R,We=performance.now()+200},Ro=()=>{window.clearTimeout(Ie),te=!0,x=c?.scrollLeft??x},Pt=()=>{window.clearTimeout(Ie),Ie=window.setTimeout(()=>{c&&(x=c.scrollLeft,x+=De(),te=!1,Ae=performance.now()+kt)},Lt)},Ao=()=>{window.clearTimeout(Be),se=!0,R=p?.scrollLeft??R},xt=()=>{window.clearTimeout(Be),Be=window.setTimeout(()=>{p&&(R=p.scrollLeft,R+=Ve(),se=!1,We=performance.now()+kt)},Lt)},Rt=e=>{const t=Math.min((e-(xe??e))/1e3,.1),s=je?xo:Pe;if(xe=e,qe+=(s-qe)*Math.min(t*6,1),c){const n=o.getBoundingClientRect(),l=n.bottom>0&&n.top<window.innerHeight;l&&!Z&&!te&&!se&&!document.documentElement.classList.contains("is-pf-modal-open")&&e>=Ae&&(x+=t*qe,c.scrollLeft=x,x+=De()),l&&p?.clientWidth&&I&&!oe&&!se&&!te&&!document.documentElement.classList.contains("is-pf-modal-open")&&e>=We&&(R-=t*Pe,p.scrollLeft=R,R+=Ve())}Re=requestAnimationFrame(Rt)};!c||Re||(xe=void 0,Re=requestAnimationFrame(Rt)),requestAnimationFrame(()=>{Mt(),$t()}),c?.addEventListener("mouseenter",()=>{je=!0}),c?.addEventListener("mouseleave",()=>{je=!1}),o.addEventListener("wheel",e=>{!c||e.ctrlKey||(Oe(),(e.shiftKey||Math.abs(e.deltaX)>Math.abs(e.deltaY))&&e.preventDefault())},{passive:!1}),c?.addEventListener("pointerdown",e=>{if(!(e.button!==0||$)){if(e.pointerType==="touch"&&W.matches){Ro();return}Z=!0,ee=!1,Tt=e.clientX,Fe=c.scrollLeft,Oe()}}),c?.addEventListener("dragstart",e=>{e.preventDefault()}),c?.addEventListener("pointermove",e=>{if(!Z)return;const t=e.clientX-Tt;!ee&&Math.abs(t)<=5||(ee||(ee=!0,c.classList.add("is-dragging"),c.setPointerCapture(e.pointerId)),c.scrollLeft=Fe-t,Fe+=De(),x=c.scrollLeft,Oe())});const qo=e=>{!c||!Z||(Z=!1,c.classList.remove("is-dragging"),c.hasPointerCapture(e.pointerId)&&c.releasePointerCapture(e.pointerId),x=c.scrollLeft,ee&&(ge=!0,window.setTimeout(()=>{ge=!1},0)))};c?.addEventListener("scroll",()=>{te&&(x=c.scrollLeft,Pt())},{passive:!0});const At=e=>{if(e.pointerType==="touch"&&W.matches){Pt();return}qo(e)};c?.addEventListener("pointerup",At),c?.addEventListener("pointercancel",At),c?.addEventListener("click",e=>{ge&&(ge=!1,e.preventDefault(),e.stopPropagation())},!0),p?.addEventListener("pointerdown",e=>{if(!(e.button!==0||$)){if(e.pointerType==="touch"&&W.matches){Ao();return}oe=!0,re=!1,Et=e.clientX,He=p.scrollLeft,Ct()}}),p?.addEventListener("dragstart",e=>{e.preventDefault()}),p?.addEventListener("pointermove",e=>{if(!oe)return;const t=e.clientX-Et;!re&&Math.abs(t)<=5||(re||(re=!0,p.classList.add("is-dragging"),p.setPointerCapture(e.pointerId)),p.scrollLeft=He-t,He+=Ve(),R=p.scrollLeft,Ct())});const jo=e=>{!p||!oe||(oe=!1,p.classList.remove("is-dragging"),p.hasPointerCapture(e.pointerId)&&p.releasePointerCapture(e.pointerId),R=p.scrollLeft,re&&(he=!0,window.setTimeout(()=>{he=!1},0)))};p?.addEventListener("scroll",()=>{se&&(R=p.scrollLeft,xt())},{passive:!0});const qt=e=>{if(e.pointerType==="touch"&&W.matches){xt();return}jo(e)};p?.addEventListener("pointerup",qt),p?.addEventListener("pointercancel",qt),p?.addEventListener("click",e=>{he&&(he=!1,e.preventDefault(),e.stopPropagation())},!0);const Fo=70,ze=14,Ne=12;let V=0,jt=P.offsetWidth,Ye=P.offsetHeight;const Ft=()=>{jt=P.offsetWidth,Ye=P.offsetHeight};document.fonts?.ready.then(Ft);const G=(e=0)=>{if(e){if(V)return;V=window.setTimeout(()=>{V=0,P.classList.remove("is-visible")},e);return}window.clearTimeout(V),V=0,P.classList.remove("is-visible")},Io=()=>{window.clearTimeout(V),V=0,P.classList.add("is-visible")},Wo=e=>{const t=jt/2,s=Math.min(Math.max(t+Ne,e.clientX),window.innerWidth-t-Ne),l=e.clientY+ze+Ye+Ne<=window.innerHeight?e.clientY+ze:e.clientY-ze-Ye;P.style.transform=`translate3d(${s}px, ${l}px, 0) translateX(-50%)`};o.addEventListener("pointermove",e=>{if(e.pointerType!=="mouse")return;Wo(e);const t=e.target instanceof Element?e.target:null,s=t?.closest(".pf-work");if(s&&o.contains(s)){Io();return}if(t&&ft?.contains(t)){G(Fo);return}G()}),o.addEventListener("pointerleave",()=>G()),window.addEventListener("blur",()=>G()),window.addEventListener("scroll",()=>G(),{passive:!0}),o.querySelectorAll(".pf-work").forEach(e=>{e.addEventListener("click",t=>{if(!nt(t,e))return;const s=a.find(n=>e.classList.contains(`pf-work--${n.id}`));s&&(J(s),G(),t.preventDefault(),dt(e,s))})}),window.addEventListener("resize",()=>{Ft(),requestAnimationFrame(()=>{Mt({preservePosition:!0}),$t({preservePosition:!0})})});const It=()=>{const e=C();if($){$.projectId!==e&&$.closeModal();return}if(!e)return;const t=a.find(n=>n.id===e),s=o.querySelector(`.pf-work--${e}:not(.pf-work--clone)`);t&&s&&dt(s,t,{pushHistory:!1})};return window.addEventListener("popstate",It),{resetModal:()=>$?.disposeModal(),syncModalWithHistory:It}},Je=document.querySelector("[data-pf]"),Kt=document.querySelector("[data-about]"),X=document.querySelector(".about-photo-stack"),Xt=document.querySelector("[data-project-list]"),Qo=document.querySelector("[data-current-project]");document.querySelector("[data-total-projects]");const Zo=document.querySelector(".counter-line"),_t=document.querySelector(".fixed-project-meta"),er=document.querySelector("[data-side-number]"),tr=document.querySelector("[data-side-wip]"),or=document.querySelector("[data-side-title]"),rr=document.querySelector("[data-counter-wip]"),Qe=document.querySelector("[data-year-rail]"),sr=[...document.querySelectorAll("[data-gnb-scroll]")],to=document.querySelector(".theme-toggle"),_=document.documentElement,oo="portfolio-last-project-slug",Ut=760,ir=12,nr=8,ro=window.matchMedia("(max-width: 600px)"),tt=window.matchMedia("(prefers-reduced-motion: reduce)"),ar=3e3,lr=200,cr={work:40,experience:-50};let Ze=!1,ce=0,Ue=0,Jt=0,so=0;const dr=()=>{const o=X?.lastElementChild;!o||X.children.length<2||o.classList.contains("is-moving-to-back")||(o.classList.add("is-moving-to-back"),so=window.setTimeout(()=>{X.prepend(o),window.requestAnimationFrame(()=>o.classList.remove("is-moving-to-back"))},lr))},io=()=>{window.clearInterval(Jt),window.clearTimeout(so),X?.querySelector(".is-moving-to-back")?.classList.remove("is-moving-to-back"),!(!X||X.children.length<2||tt.matches)&&(Jt=window.setInterval(dr,ar))},pr=o=>{try{return window.sessionStorage?.getItem(o)||""}catch{return""}},ur=(o,a)=>{try{window.sessionStorage?.setItem(o,a)}catch{}},fr=o=>{window.gtag?.("event","project_click",{project_name:o})},mr=o=>`'${String(o).slice(-2)}`,gr=o=>{const{start:a,end:u}=eo.yearRange,h=Math.min(5,u-a+1),S=Math.min(Math.max(o-2,a),u-h+1);return Array.from({length:h},(U,O)=>S+O)},hr=o=>o.split("|").join(" "),wr=(o,a)=>o.button===0&&!o.defaultPrevented&&!o.metaKey&&!o.ctrlKey&&!o.shiftKey&&!o.altKey&&!a.target&&!a.hasAttribute("download"),vr=()=>window.matchMedia("(max-width: 600px)").matches?nr:ir,yr=()=>{const o=vr();return{top:o,left:o,width:window.innerWidth-o*2,height:window.innerHeight-o*2}},br=o=>{const a=o.getBoundingClientRect(),u=yr(),h=window.getComputedStyle(o),S=window.getComputedStyle(o.querySelector(".project-media")),U=o.querySelector(".project-summary"),O=Number.parseFloat(h.rowGap||h.gap)||0,$=U?.getBoundingClientRect().height||0,de=Math.max(0,u.height-$-O),J=document.createElement("div"),C=o.cloneNode(!0);return C.classList.add("project-transition-card"),C.removeAttribute("href"),C.removeAttribute("aria-label"),C.setAttribute("aria-hidden","true"),Object.assign(C.style,{top:`${a.top}px`,left:`${a.left}px`,width:`${a.width}px`,height:`${a.height}px`,gap:h.gap,gridTemplateRows:h.gridTemplateRows,maxHeight:"none"}),C.style.setProperty("--project-media-shadow",S.boxShadow),J.className="project-transition-layer",J.append(C),document.body.append(J),{cardClone:C,targetRect:u,targetGridTemplateRows:`${de}px ${$}px`}},Sr=o=>{if(Ze)return;const a=o.closest(".project-section");if(!a){window.location.href=o.href;return}Ze=!0,a.classList.add("is-transition-source"),document.body.classList.add("is-project-transitioning"),document.documentElement.classList.add("is-project-transitioning");const{cardClone:u,targetRect:h,targetGridTemplateRows:S}=br(o);u.animate([{top:u.style.top,left:u.style.left,width:u.style.width,height:u.style.height,gridTemplateRows:u.style.gridTemplateRows,opacity:1,offset:0},{opacity:1,offset:.9},{top:`${h.top}px`,left:`${h.left}px`,width:`${h.width}px`,height:`${h.height}px`,gridTemplateRows:S,opacity:0,offset:1}],{duration:Ut,easing:"cubic-bezier(0.16, 1, 0.3, 1)",fill:"forwards"}),window.setTimeout(()=>{window.location.href=o.href},Ut-120)},Lr=()=>{Ze=!1,document.documentElement.classList.remove("is-project-transitioning"),document.body.classList.remove("is-project-transitioning"),document.querySelectorAll(".project-transition-layer").forEach(o=>o.remove()),document.querySelectorAll(".is-transition-source").forEach(o=>{o.classList.remove("is-transition-source")})},kr=o=>{Qe.innerHTML=gr(o.year).map(a=>{const u=a===o.year;return`
        <span class="year-rail-item" data-year="${a}" ${u?'aria-current="true"':""}>${mr(a)}</span>
      `}).join("")},Tr=()=>{_t.classList.add("is-changing"),Qe.classList.add("is-changing"),window.setTimeout(()=>{_t.classList.remove("is-changing"),Qe.classList.remove("is-changing")},180)};let Te=[],Qt=-1;const ot=({force:o=!1}={})=>{if(!ro.matches){ce&&(_.style.removeProperty("--mobile-project-viewport"),ce=0,Ue=0);return}const a=window.innerWidth;!o&&ce&&a===Ue||(ce=window.innerHeight,Ue=a,_.style.setProperty("--mobile-project-viewport",`${ce}px`))},Er=()=>{if(!Je)return;const o=Je.getBoundingClientRect(),a=window.innerHeight/2,u=o.top<=a&&o.bottom>=a;_.classList.toggle("is-pf-active",u)},Mr=()=>{if(!Kt)return;const o=Kt.getBoundingClientRect(),a=window.innerHeight/2,u=o.top<=a&&o.bottom>=a;_.classList.toggle("is-about-active",u)},$r=o=>{if(o===Qt||o<0)return;const a=_o[o],u=(o+1)/eo.totalProjectCount*100;Qt=o,Tr(),Qo.textContent=a.number,er.textContent=a.number,tr.textContent=a.wip?"🚧":"",rr.textContent=a.wip?"🚧":"",or.innerHTML=a.sideTitle.split("|").join("<br />"),Zo.style.setProperty("--progress",`${u}%`),ur(oo,a.slug),kr(a)},Cr=()=>{if(!Te.length)return;const o=window.innerHeight/2,a=Te.map((u,h)=>{const S=u.getBoundingClientRect(),U=S.top+S.height/2;return{index:h,distance:Math.abs(U-o)}}).sort((u,h)=>u.distance-h.distance)[0]?.index;Te.forEach((u,h)=>{const S=h===a;u.classList.toggle("is-active",S),u.classList.toggle("is-inactive",!S)}),$r(a)},Pr=o=>{const a=Math.max(0,document.documentElement.scrollHeight-window.innerHeight),u=cr[o.id]??0,h=o===document.body?0:o.getBoundingClientRect().top+window.scrollY-u,S=Math.min(a,Math.max(0,h));window.scrollTo({top:S,behavior:tt.matches?"auto":"smooth"})};let Zt=0;const Ee=()=>{cancelAnimationFrame(Zt),Zt=requestAnimationFrame(()=>{Er(),Mr(),Cr()})},no=o=>{_.dataset.theme=o,to.setAttribute("aria-pressed",String(o==="dark")),localStorage.setItem("portfolio-theme",o)},xr=localStorage.getItem("portfolio-theme")||"light",ao=()=>window.performance.getEntriesByType("navigation")[0]?.type||"navigate";tt.addEventListener?.("change",io);io();const et=Jo(Je,Uo,hr);Te=[...document.querySelectorAll("[data-project]")];ot({force:!0});no(xr);const Rr=()=>{const o=window.location.hash?window.location.hash.slice(1):"",a=ao()==="back_forward",u=o||(a?pr(oo):""),h=u?document.getElementById(u):null;o&&window.history.replaceState(null,"",`${window.location.pathname}${window.location.search}`),h&&h.scrollIntoView({block:"center",behavior:"auto"})};Rr();ao()==="reload"&&window.requestAnimationFrame(()=>window.scrollTo({top:0,behavior:"auto"}));to.addEventListener("click",()=>{no(_.dataset.theme==="dark"?"light":"dark")});sr.forEach(o=>{o.addEventListener("click",a=>{const u=document.querySelector(o.hash);u&&(a.preventDefault(),Pr(u))})});Xt?.addEventListener("click",o=>{const a=o.target.closest(".project-link");!a||!Xt.contains(a)||!wr(o,a)||(fr(a.closest(".project-section")?.dataset.analyticsId),o.preventDefault(),Sr(a))});window.addEventListener("scroll",Ee,{passive:!0});window.addEventListener("resize",()=>{ot(),Ee()});ro.addEventListener?.("change",()=>{ot({force:!0}),Ee()});window.addEventListener("pagehide",()=>et?.resetModal());window.addEventListener("pageshow",()=>{et?.resetModal(),et?.syncModalWithHistory(),Lr()});Ee();
